...
class LikeRegisters {
  ...
  largest(input) {
    // 1. split lines
    // 2. hash for registers, with value
    // 3. sort for largest
    // 4. return the largest in hash value array
    ...
    for (let line of lines) {
      ...
      // console.log(items)
      if (!hash.hasOwnProperty(items[0])) hash[items[0]] = 0
      if (!hash.hasOwnProperty(items[4])) hash[items[4]] = 0

      switch(items[5]) {
        case '>':
          if (hash[items[4]] > parseInt(items[6])) {
            if (items[1] === 'inc') hash[items[0]] += parseInt(items[2])
            else if (items[1] === 'dec') hash[items[0]] -= parseInt(items[2])
          }
          break
        case '<':
          if (hash[items[4]] < parseInt(items[6])) {
      ...
      // console.log(Object.values(hash).sort())
    }
    const sorted_values = Object.values(hash).sort()
    return sorted_values[sorted_values.length-1]
  }

  largest_ever(input) {
    // 1. split lines
    // 2. compute for registers, hash with value
    // 3. record each step for max value
    ...
    for (let line of lines) {
      ...
      switch(items[5]) {
        case '>':
          ...
          if (max < hash[items[0]]) max = hash[items[0]]
          break
        case '<':
          if (hash[items[4]] < parseInt(items[6])) {
            if (items[1] === 'inc') hash[items[0]] += parseInt(items[2])
...
if (require.main === module) {
  const register = new LikeRegisters()

  let input = `b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10`
  let output = register.largest(input)
  console.log(output)
  assert.equal(output, 1)

  output = register.largest_ever(input)
  console.log(output)
  assert.equal(output, 10)

  console.log('======')

  input = `o inc 394 if tcy >= -3
s inc -204 if s <= -5
keq inc -34 if keq > -6
ip inc 762 if ip == 4
zr inc -644 if ip >= 6
...
pc dec 916 if vt <= 426`
  output = register.largest(input)
  console.log(output)
  assert.equal(output, 5752)

  output = register.largest_ever(input)
  console.log(output)
  assert.equal(output, 6366)
}