...
class Passphrase {
  check_valid(paragraph) {
    // 1. split to lines
    // 2. splits each lines to words
    // 3. put words to hash for each line
    // 4. check the hash to ensure without duplicate words
    //
    //
    // Step 1.
    ...
    for(let l = 0; l < lines.length; ++l) {
      ...
      for (; i < words.length; ++i) {
        // Step 4.
        if (hash.hasOwnProperty(words[i])) break
        else {
          hash[words[i]] = 1
        }
      }
  ...
  check_valid_without_anagrams(paragraph) {
    // 1. split to lines
    // 2. splits each lines to words
    // 3. put words with sorted to hash for each line
    // 4. check the hash without duplicate words
    //
    //
    // Step 1.
    const lines = paragraph.split(/\n/)
    ...
    for(let l = 0; l < lines.length; ++l) {
      ...
      let i = 0
      for (; i < words.length; ++i) {
        const word_sorted = words[i].split('').sort().toString()
        if (hash.hasOwnProperty(word_sorted)) break
        else {
          hash[word_sorted] = 1
        }
      }
...
if (require.main === module) {
  ...
  let para = `aa bb cc dd ee
aa bb cc dd aa
aa bb cc dd aaa
aa bb cc dd aaa bb
aa bb cc dd dd`
  let r = passphrase.check_valid(para)
  // console.log(r)
  assert.equal(r, 2)
...
oiii ioii iioi iiio`
  r = passphrase.check_valid_without_anagrams(para)
  // console.log(r)
  assert.equal(r, 3)

  para = `bdwdjjo avricm cjbmj ran lmfsom ivsof
