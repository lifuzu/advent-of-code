...
class RecursiveCircus {
  constructor() {}
  scan(input) {
    // 1. split if having subtree
    // 2. only scan the line having parent AND children
    // 3. put children into an array
    // 4. put parent into another array
    // 5. if some parents can be found in children array, just remove it from parent array, and rescan
    // 6. return the only item in the array
    ...
    for (let i = 0; i < parent_array.length; ++i) {
      if (children_array.includes(parent_array[i])) {
        // console.log(parent_array[i])
        parent_array.splice(i, 1)
        i--
      }
    }
    // 6. finally, it should be only one left in the parent array
    // assert tree_array.length === 1
    return parent_array[0]
  }

  difference(input) {
    // 1. split if having subtree
    // 2. build them into a tree with multiple level of children
    // 3. have the root of the tree
    // 4. sum the val to node recursively, keep the original val as well
    // 5. traversal children to find the different value at some level
    ...
    for (let line of lines) {
      ...
      // 1st scan to get all single node
      if (children === undefined) {
        let tmp = parent.split(/ /)
        let child_name = tmp[0]
        let child_val = parseInt(tmp[1].replace('(', '').replace(')', ''))
        child_obj[child_name] = new Node(child_name, child_val)
      }
    }
    for (let line of lines) {
      let [parent, children] = line.split(/->/)
      // 2nd scan to get all node relationship
      if (children !== undefined) {
        let tmp = parent.split(/ /)
        let parent_name = tmp[0]
        let parent_val = parseInt(tmp[1].replace('(', '').replace(')', ''))
        children = children.trim().split(/, /)

        const obj = { 'name': parent_name, 'val': parent_val, 'children': children}
    ...
    let n = 0
    const parent_obj = {}
    const parent_array_length = parent_array.length
    while (n < parent_array_length) {
      for (let p of parent_array) {
        // console.log(p)
        let node = null
        if (!parent_obj.hasOwnProperty(p.name)) {
          node = new Node(p.name, p.val)
          parent_obj[p.name] = node
        } else {
          node = parent_obj[p.name]
        }

        for (let c of p.children) {
          // if some of children can be found, put them as children of tree
          if (child_obj.hasOwnProperty(c)) {
            node.children.push(child_obj[c])
            // remove it from the children array
            p.children.splice(p.children.indexOf(c), 1)
          }
          // find the children from the parent object hash
          if (parent_obj.hasOwnProperty(c) && parent_array.filter( el => el.name === c).length === 0) {
            node.children.push(parent_obj[c])
            // remove from parent obj
            delete parent_obj[c]
            p.children.splice(p.children.indexOf(c), 1)
          }
        }
        // if all children set as children of tree, remove the item from parent array
        if (p.children.length === 0) {
          parent_array.splice(parent_array.indexOf(p), 1)
          n++
        }
      }
    }
    ...
    // 3. have the root of the tree
    const root_key = Object.keys(parent_obj)[0]
    // 4. sum the val to node recursively, keep the original val as well
    this.set_val_recursively(parent_obj[root_key])
    // 5. traversal children to find the different value at some level
    let [base, diff, diff_obj] = this.traversal_level(parent_obj[root_key])

    // 6. return the difference
    return base - diff + diff_obj.orig_val
  }
  set_val_recursively(root) {
    if (root.children.length === 0) return root.val
    else {
      let sum = 0
      for (let child of root.children) {
        sum += this.set_val_recursively(child)
      }
      root.orig_val = root.val
      root.val = sum + root.orig_val
      return root.val
    }
  }
  traversal_level(root) {
    ...
    if (root.children.length !== 0) {
      for (let i = 0; i < root.children.length - 2; ++i) {
        let one = root.children[i].val, two = root.children[i+1].val, three = root.children[i+2].val
        // if some one if different, go deep into it
        if (one === two && two !== three) return this.traversal_level_recursively(one, three, root.children[i+2])
        else if (one === three && two !== three) return this.traversal_level_recursively(one, two, root.children[i+1])
        else if (two === three && one !== two ) return this.traversal_level_recursively(two, one, root.children[i])
      }
    }
  }

  traversal_level_recursively(base, diff, diff_obj) {

    const children = diff_obj.children
    if (children.length !== 0) {
      let i = 0
      for (; i < children.length - 2; ++i) {
        let one = children[i].val, two = children[i+1].val, three = children[i+2].val
        // if some one if different, go deep into it, recursively
        if (one === two && two !== three) return this.traversal_level_recursively(one, three, children[i+2])
        else if (one === three && two !== three) return this.traversal_level_recursively(one, two, children[i+1])
        else if (two === three && one !== two ) return this.traversal_level_recursively(two, one, children[i])
      }
      // if all children val are same, return passed parameter, which is parent info
      if (i === children.length - 2) {
        return [base, diff, diff_obj]
      }
    }
  }
}
...
if (require.main === module) {
  const recursiveCircus = new RecursiveCircus()
...
gyxo (61)
cntj (57)`
  let output = recursiveCircus.scan(input)
  console.log(output)
  assert.equal(output, 'tknk')

  output = recursiveCircus.difference(input)
  console.log(output)
...
uhcach (18)`
  output = recursiveCircus.scan(input)
  console.log(output)
  assert.equal(output, 'mkxke')

  output = recursiveCircus.difference(input)
  console.log(output)
  assert.equal(output, 268)
}