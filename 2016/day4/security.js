...
class SecurityThroughObscurity {
  constructor() {}
  sum(input) {
    // 1. split to encryped name, id, and checksum
    // 2. if the checksum validated with encrypted name, sum += parseInt(id)
    // 3. return sum
    ...
    for (let line of lines) {
      ...
      // console.log(tmp)
      const tmp_arr = tmp[0].split(/-/)
      const encrypted = tmp_arr.slice(0, -1).join('')
      const id = parseInt(tmp_arr.slice(-1))
      const checksum = tmp[1].replace(']', '')

      // console.log(encrypted, id, checksum)
      const hash = {}
      for (let i = 0; i < encrypted.length; ++i) {
        // console.log(encrypted[i])
        if (!hash.hasOwnProperty(encrypted[i])) hash[encrypted[i]] = 1
        else hash[encrypted[i]]++
      }
      // console.log(hash)
  ...
  shift_cipher(input) {
    // 1. split to encryped name, id, and checksum
    // 2. calculate remainder of parseInt(id) % 26
    // 3. rotate the every charactor by the remainder, over z, go to a
    // 3. return the decryped str
    ...
    for (let line of lines) {
      ...
      const tmp_arr = tmp[0].split(/-/)
      const encrypted = tmp_arr.slice(0, -1).join('')
      const encrypted_orig = tmp_arr.slice(0, -1).join('-')
      const sector_id = parseInt(tmp_arr.slice(-1))
      const checksum = tmp[1].replace(']', '')

      const remainder = sector_id % 26
      ...
      if (str === checksum) {
        let decryped = ''
        for (let i = 0; i < encrypted_orig.length; ++i) {
          // if (encrypted[i] === '-') {
          //   decryped += ' '
          //   continue
          // }
          ...
          // console.log(encrypted_orig[i])
          const p = alphabet.indexOf(encrypted_orig[i])
          if (p + remainder >= 26) decryped += alphabet[p + remainder - 26]
          else if (p + remainder < 26) decryped += alphabet[p + remainder]
        }
        if (decryped.indexOf('Pole'.toLowerCase()) !== -1) {
          console.log(decryped)
          return sector_id
        }
      }
...
if (require.main === module) {
  const security = new SecurityThroughObscurity()

  let input = `aaaaa-bbb-z-y-x-123[abxyz]
a-b-c-d-e-f-g-h-987[abcde]
not-a-real-room-404[oarel]
totally-real-room-200[decoy]`
  let output = security.sum(input)
  console.log(output)
  assert.equal(output, 1514)

  input = `qzmt-zixmtkozy-ivhz-343[zimth]`
  output = security.shift_cipher(input)
...
bxaxipgn-vgpst-qphzti-rdcipxcbtci-635[ipctx]`
  output = security.sum(input)
  console.log(output)
  assert.equal(output, 361724)

  output = security.shift_cipher(input)
  console.log(output)
  assert.equal(output, 482)
}